...
function buildConsoleTransport(
  ...
  if (isDevMode) {
    return {
      ...
      // (`2:00PM`) and one line per record. Kept to plain string options so the
      // whole thing serializes into the pino-pretty worker thread — no
      // functions, no custom stream.
      options: {
        colorize: true,
